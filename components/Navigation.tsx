    ...
    <>
      ...
      <nav className={`fixed top-0 w-full z-[100] transition-all duration-500 ease-in-out ${scrolled ? 'bg-black/80 backdrop-blur-xl py-4 border-b border-white/5' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center relative z-[101]">
          <a href="#" className="text-2xl font-bold tracking-widest text-white uppercase hover:scale-105 transition-transform cursor-pointer">
            TC Studios
          </a>
          ...
          <button 
            ref={toggleRef}
            className="md:hidden text-white active:scale-90 transition-transform p-2"
        ...
        {/* Mobile Nav Overlay */}
        {mobileMenuOpen && (
          <div className="fixed inset-0 bg-black/95 flex flex-col items-center justify-center space-y-8 z-[100] animate-in fade-in zoom-in-95 duration-300">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href}
